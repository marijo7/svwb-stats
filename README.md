...
- **Windows のファイアウォール** — 初回起動時に Python の通信を許可するか聞かれる。「プライベートネットワーク」にチェックを入れて許可しないとスマホから繋がらない。誤って拒否した場合は、Windows セキュリティ → ファイアウォールとネットワーク保護 → アプリにファイアウォール経由の通信を許可する から変更できる。
